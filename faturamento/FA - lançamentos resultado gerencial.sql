    select distinct
        ...
        case when nullif(ZC1.ZC1_FILIAL, '') is not null then 'P |01|01'+ CAST(ZC1.ZC1_FILIAL AS CHAR (6)) else 'P |01||' end as BK_FILIAL,
    ...
    /*
        ...
        */
        ...
        trim(ZC1.ZC1_FILIAL) as FILIAL_VGOS,
        trim(ZC1.ZC1_NUM) as NUM_VGOS,
    ...
    where
            ZE3.ZE3_COMPET=:PERIODO and ZE3_ITEMPL like '05%'
        and ZE3.D_E_L_E_T_ = ''
union
    select distinct
        'P |01|01' AS BK_EMPRESA,
        ...
        'P |01|SA1010|'+ COALESCE(NULLIF(RTRIM(COALESCE(SA1.A1_FILIAL, ' '))+'|'+RTRIM(COALESCE(SA1.A1_COD, ' '))+RTRIM(COALESCE(SA1.A1_LOJA, ' ')), ' '), '|') as BK_CLIENTE,
        null as BK_FORNECEDOR,
        null as ID_OSPORTUARIA,
        concat(trim(DUD.DUD_FILORI), trim(DUD.DUD_VIAGEM)) as ID_VIAGEMTMS,
        null as ID_MERCADORIA,
        null as ID_PEDIDODEVENDA,
        null as ID_NFS,
        null AS BK_NAT_FINANCEIRA,
        null AS BK_CONDICAO_DE_PAGAMENTO,
        case when nullif(concat(trim(DUD.DUD_FILORI), trim(DUD.DUD_VIAGEM)), trim(DUD.DUD_FILORI)) is not null then ('P |01|CTD010|'+ COALESCE(NULLIF(RTRIM(COALESCE('', ' '))+'|'+RTRIM(COALESCE('11', ' ')), ' '), '|')) else null end as BK_ITEM_CONTABIL,
        'P |01|CTT010|'+ COALESCE(NULLIF(RTRIM(COALESCE(CTT010.CTT_FILIAL, ' '))+'|'+RTRIM(COALESCE(ZE3.ZE3_ORIGEM, ' ')), ' '), '|') as BK_CENTRO_DE_CUSTO,
        null as ID_TABELA_PRECO,
        concat(ZE3.ZE3_COMPET, '01') as PERIODO,
        concat(ZE3.ZE3_COMPET, '01') as COMPETENCIA,
        ZE2.ZE2_COD as CONTAROP,
        ZE2.ZE2_CONTA as CONTA_CONTABIL,
    ...
    /*
        ...
        */
        ...
        /* para validação no RM */
        trim(ZE3.ZE3_NUM) as OS_VGA,
        trim(DUD.DUD_FILORI) as FILIAL_VGOS,
        trim(DUD.DUD_VIAGEM) as NUM_VGOS,
    ...
    from ZE3010 ZE3
        ...
        left join DUD010 DUD
            ...
            (
                ...
                left join SA1010 SA1 (nolock)
                    ...
                    and
                    (
                        concat(SA1.A1_COD, SA1.A1_LOJA) = concat(SD2.CTE_CLIENTE, SD2.CTE_LOJA) or
                        concat(SA1.A1_COD, SA1.A1_LOJA) = concat(SD2.CMP_CLIENTE, SD2.CMP_LOJA) or
                        concat(SA1.A1_COD, SA1.A1_LOJA) = concat(SD2.RPS_CLIENTE, SD2.RPS_LOJA)
                    )
        ...
        left join CTT010
            ...
            and CTT010.CTT_CUSTO = ZE3.ZE3_ORIGEM
    where
            ZE3.ZE3_COMPET=:PERIODO and ZE3_ITEMPL like '051%'
        and ZE3.D_E_L_E_T_ = ''
