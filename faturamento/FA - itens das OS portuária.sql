...
union
    select
        ...
        coalesce
        (
            case when cast(ZC2.ZC2_TIPO as int) = 4 then 'P |01|SB1010|'+ COALESCE(NULLIF(RTRIM(COALESCE(SB1.B1_FILIAL, ' '))+'|'+RTRIM(COALESCE(ZC2.ZC2_COD, ' ')), ' '), '|') else null end,
            case when cast(ZC2.ZC2_TIPO as int) = 11 then coalesce('P |01|SB1010|'+ COALESCE(NULLIF(RTRIM(COALESCE(SB1.B1_FILIAL, ' '))+'|'+RTRIM(COALESCE(ZC2.ZC2_COD, ' ')), ' '), '|'), ) else null end
        ) as COD_SB1,
