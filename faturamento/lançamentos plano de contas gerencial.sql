select distinct
    CASE WHEN ZC2.FILIAL IS NULL THEN 'P |01||' ELSE 'P |01|01'+ CAST(ZC2.FILIAL AS CHAR (6)) END AS BK_FILIAL,
    ...
    case when len(ZE2.ZE2_COD) = 8 and left(ZE2.ZE2_COD, 2) = '01' then ZE3.ZE3_VALOR when len(ZE2.ZE2_COD) = 8 and left(ZE2.ZE2_COD, 2) like '_[2-9]' then ZE3.ZE3_VALOR*-1 else 0.0 end as VALOR,
    ZC2.*,
...
from ZE3010 ZE3
    ...
    (
        select
            ...
            left(nullif(ZC2010.ZC2_COMPET, ''), 6) as PERIODO,
    ...
    on concat(ZC2.FILIAL, ZC2.OS) = ZE3.ZE3_NUM
