SELECT 'P |01|01' AS BK_EMPRESA,
       ...
       year(SF2.F2_EMISSAO) as ANO_EMISSAO,
       month(SF2.F2_EMISSAO) as MES_EMISSAO
FROM SD2010 SD2
...
AND A3_COD = F2_VEND1
AND SA3.D_E_L_E_T_ = ' '
LEFT JOIN CTD010 CTD ON CTD_FILIAL = '      '
AND CTD_ITEM = D2_ITEMCC
AND CTD.D_E_L_E_T_ = ' '
LEFT JOIN SX5010 CFOP ON X5_FILIAL = '      '
AND X5_TABELA = '13'
AND X5_CHAVE = D2_CF
AND CFOP.D_E_L_E_T_ = ' '
...
AND SE4.D_E_L_E_T_ = ' '
LEFT JOIN ACY010 ACY ON ACY_FILIAL = SUBSTRING(D2_FILIAL, 1, 4)
AND ACY_GRPVEN = A1_GRPVEN
AND ACY.D_E_L_E_T_ = ' '
LEFT JOIN SAH010 SAH ON AH_FILIAL = '      '
AND AH_UNIMED = D2_UM
AND SAH.D_E_L_E_T_ = ' ' WHERE  D2_TIPO NOT IN ('B', 'D' ) AND SD2.D_E_L_E_T_ = ' ' AND SD2.D2_SERIE NOT IN ('003','100') 