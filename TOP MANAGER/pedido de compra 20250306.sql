...
select distinct -- PEDIDO DE COMPRA
 ...
 Autorizador = case
                   when Cop.FlCopAut = 0 then 'Pedido NÃO Autorizado'
                   else Isnull(Atd.NmPes, 'Pedido Autorizado')
               end,
               Comprador = case
                               when Cop.CdCocufnOri is null then ''
                               else Scp.Comprador--.NmUsr

                           end ,
                           DtCotacao = Scp.DtCotacao --Coc.DtCoc
--,   AprovadorSolicitacao = Scp.AprovadorSolicitacao
,
                           AprovadorSolicitacao = case
                                                      when (Scp.AprovadorSolicitacao is null
                                                            and Scp.FlScpAut = 1) then 'Solicitação Aprovada Automaticamente'
                                                      else Scp.AprovadorSolicitacao
                                                  end ,
                                                  DhAprovadorSolicitacao = case
                                                                               when (Scp.DhAprovadorSolicitacao is null
                                                                                     and Scp.FlScpAut = 1
                                                                                     and Scp.DhAprovadorSolicitacao = '1900-01-01 00:00:00.000') then ' '
                                                                               else Scp.DhAprovadorSolicitacao
                                                                           end -- TOTALIZADORES PEDIDO DE COMPRA
,
...
from TbCopIte CopIte
    ...
    inner join TbCop Cop on Cop.CdCop = CopRem.CdCop
        ...
        left join TbUsr UsrRes on UsrRes.CdUsr = Cpd.CdUsrRes -- COMPROMISSO DE COMPRA Alex 23/08
        --left join	TbCocUfn CocUfn on Cocufn.CdCocufn = Cop.CdCocufnOri
        --left join   TbCocUne CocUne on CocUne.CdCocune = CocUfn.CdCocune
        ...
        (select Coprem.CdCop ,
                ...
                Scp.FlScpAut
        from TbCopite Copite
        join TbCoprem Coprem on Coprem.CdCoprem = Copite.CdCoprem
        join TbCocsca Cocsca on Cocsca.CdCocufo = Copite.CdCocufoOri
        join TbCocscp Cocscp on Cocscp.CdCocscp = Cocsca.CdCocscp
        join TbScp Scp on Scp.CdScp = Cocscp.CdScp
        join TbUsr UsrScp on UsrScp.CdUsr = Scp.CdUsr
        join TbCoc Coc on Coc.CdCoc = Cocscp.CdCoc
        join TbUsr UsrCom on UsrCom.CdUsr = Coc.CdUsrRes
            left join TbAtd Atd on Atd.NrAtdRef = Scp.CdScp
        ...
        group by Coprem.CdCop,
                Scp.FlScpAut) Scp on Scp.CdCop = Cop.CdCop -- Dados Solicitação, requisitante e data de requisição
    --Adicionado 04/03/2013 chamado:18331
    ...
    -- EMPRESA
    join TbUne Une on Une.CdUne = Cop.CdUne
    join
        (select Pes.CdPes ,
                Pes.NmPes ,
                Tlg.SgTlg ,
                ...
                Pes.NrPesEdrCom ,
                Pes.NrPesEdrCep ,
                NmLocBai = (case Loc.TpLoc
                                when 5 then Loc.NmLoc
                            end) ,
                NmLocCid = (case LocCid.TpLoc
                                when 4 then LocCid.NmLoc
                                else Loc.NmLoc
                            end) ,
                NrLocCidDdd = (case LocCid.TpLoc
                                when 4 then LocCid.NrLocDdd
                                else Loc.NrLocDdd
                            end) ,
                SgLocEst = (case LocEst.TpLoc
                                when 3 then LocEst.SgLoc
                                else LocCid.SgLoc
                            end) ,
                Pes.TpPes ,
                Pes.NrPesCpj ,
                Pes.NrPesCgf
        from TbPes Pes
        join TbLlg Llg on Llg.CdLlg = Pes.CdLlg
        join TbLoc Loc on Loc.CdLoc = Llg.CdLoc
        join TbLgr Lgr on Lgr.CdLgr = Llg.CdLgr
        join TbTlg Tlg on Tlg.CdTlg = Lgr.CdTlg
        join TbLoc LocCid on LocCid.CdLoc = Loc.CdLocMae
        join TbLoc LocEst on LocEst.CdLoc = LocCid.CdLocMae) PesUne on PesUne.CdPes = Une.CdPes
        left join
        (select Top 1 Pes.CdPes ,
                    Mct.NrMctTel ,
                    Mct.NmMct
        from TbPes Pes
        join TbMct Mct on Mct.CdPes = Pes.CdPes
        join TbTmc Tmc on Tmc.CdTmc = Mct.CdTmc
        where Tmc.TpTmc = 1 -- Telefone

        order by Mct.CdMct) MctUne on MctUne.CdPes = PesUne.CdPes
        left join
        (select Top 1 Pes.CdPes ,
                    Mct.NrMctEnd
        from TbPes Pes
        ...
        order by Mct.CdMct) MctUne002 on MctUne002.CdPes = PesUne.CdPes -- FORNECEDOR
    join TbFrn Frn on Frn.CdFrn = Cop.CdFrn
    join
        (select Pes.CdPes ,
                Pes.NmPes ,
                Tlg.SgTlg ,
        ...
        and Mct.CdTmc = 28) PesFrn on PesFrn.CdPes = Frn.CdPes
        join TbFrn FrnMae on FrnMae.CdFrn = Frn.CdFrnMae --left join	(Select	Top 1
    --				Pes.CdPes
    --			,	Mct.NrMctTel
    --			,	Mct.NmMct
    --			From		TbPes Pes
    ...
    --			join		TbTmc Tmc		on Tmc.CdTmc = Mct.CdTmc
    --			Where Tmc.TpTmc = 1 -- Telefone
    --			Order by Mct.CdMct) MctFrn 	on MctFrn.CdPes = PesFrn.CdPes
    -- ITEM
    join TbObj Obj on Obj.CdObj = CopIte.CdObj
    join TbUap Uap on Uap.CdUap = CopIte.CdUap
    join TbUnd Und on Und.CdUnd = Uap.CdUnd
