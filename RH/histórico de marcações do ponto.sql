select 
    ...
    cast(SPG.PG_DATA as date) as DATA_MARCA,
    SPG.TIPO,
    SPG.SEQ_APONT,
    SPG.ORDEM_APONT,
    SPG.HORA_APONT,
...
from
    (
        select
            ...
            datetimefromparts(YEAR(CAST(SPG010.PG_DATA AS DATE)), MONTH(CAST(SPG010.PG_DATA AS DATE)), DAY(CAST(SPG010.PG_DATA AS DATE)), CAST(FLOOR(SPG010.PG_HORA) AS INT), CASE WHEN SPG010.PG_HORA - FLOOR(SPG010.PG_HORA) = 0 THEN 0 ELSE CAST(ROUND((SPG010.PG_HORA - FLOOR(SPG010.PG_HORA)) * 100, 0) AS INT) END, 0, 0) as APONTAMENTO,
            lag(datetimefromparts(YEAR(CAST(SPG010.PG_DATA AS DATE)), MONTH(CAST(SPG010.PG_DATA AS DATE)), DAY(CAST(SPG010.PG_DATA AS DATE)), CAST(FLOOR(SPG010.PG_HORA) AS INT), CASE WHEN SPG010.PG_HORA - FLOOR(SPG010.PG_HORA) = 0 THEN 0 ELSE CAST(ROUND((SPG010.PG_HORA - FLOOR(SPG010.PG_HORA)) * 100, 0) AS INT) END, 0, 0)) over(partition by SPG010.PG_FILIAL, SPG010.PG_MAT, SPG010.PG_DATAAPO order by SPG010.PG_DATAAPO, SPG010.PG_TPMARCA) as APONT_ANT,
...
where SPG.PG_DATA between :DTINI_MARCA and :DTFIM_MARCA
