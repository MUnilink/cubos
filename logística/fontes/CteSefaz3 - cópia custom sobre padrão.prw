...
Local nFEmDcAnt		:= 0
Local cUFDESGLOB	:= ''
// Alterado por Paulo Amorim
// Data: 24/12/2020
// Objetivo: averbação do valor do Container
// Inicio da Alteração
Local nValCont := 0
// Fim Alteração
Private aUF       	:= {}
				...
				While !(cAliasDT6)->(Eof()) // (02)
					...
					If Empty((cAliasDT6)->DT6_CHVCTE)  
						...
						cChvAcesso := CTeCHVAC( cCodUF,;
												...
												cTpEmis + cCT)
					Else
						If Substr(AllTrim((cAliasDT6)->DT6_CHVCTE), 35, 1) $ '4;5' .And. cTpEmis == '1'
					...
					If !Empty(cSeqEntr)
						...
						Else
							DUL->(DbSetOrder(1))
								If DUL->(MsSeek(xFilial("DUL") + (cAliasDT6)->DT6_SQEDES)) .AND. (cAliasDT6)->DT6_CDRDES != DUL->DUL_CDRDES
					...
					If !Empty(cDTCObs) .Or. (cAliasDT6)->DT6_VALIMP > 0 .Or. SF2->F2_TOTIMP > 0
						...
						dbSelectArea("DTC")
						dbSetOrder(3)
						If dbSeek(XFilial("DTC")+(cAliasDT6)->DT6_FILDOC+(cAliasDT6)->DT6_DOC+(cAliasDT6)->DT6_SERIE)
							If  ALLTRIM(DTC->DTC_YTPEQP) <> ""
								dbSelectArea("SX5")
								SX5->(dbSetOrder(1))
								If SX5->(dbSeek(xFilial("SX5")+"_3" + DTC->DTC_YTPEQP))
									nValCont := VAL(SX5->X5_DESCRI)
								EndIf
							EndIf    
						EndIf
						...
						If SF2->F2_TOTIMP > 0
							...
							If nValCont > 0
								...
								cString += '<xTexto>' + ConvType(nValCont, 15, 2) + '</xTexto>'
								cString +=  '</ObsCont>'
							EndIf
							//Fim da Alteração		
							cString += '</compl>'						
					...
					If	!AllTrim((cAliasDT6)->DT6_DOCTMS) == StrZero( 8, Len(DT6->DT6_DOCTMS)) .And.;
						...
						If (cTipoNF $ '5/2/9/A') .And. (!Empty(cCtrDpc) .Or. !Empty(cCTEDocAnt))
							...
							cQuery += " SELECT DISTINCT DTC.DTC_CTEANT, SA1.A1_CGC, SA1.A1_INSCR, SA1.A1_EST, SA1.A1_CONTRIB, SA1.A1_NOME, SA1.A1_PESSOA " + CRLF
			...
			If (!(cAliasDT6)->(Eof())) // (01)
				...
				While !(cAliasDT6)->(Eof()) // (02)
					...
					cString += '<dhEmi>'+ SubStr(DToS(SF1->F1_EMISSAO), 1, 4) + "-";
                                        ...
                                        + SubStr(AllTrim(SF1->F1_HORA), 1, 2) + ":";
					                    + SubStr(AllTrim(SF1->F1_HORA), 4, 2) + ":";
                                        + SubStr(AllTrim(SF1->F1_HORA), 7, 2);
					...
					Else
						...
						If lRecebedor
							...
							cLojRec := (cAliasDT6)->( DT6_LOJREC )
						EndIf
					cStringExp	:= grpExpedid((cAliasDT6)->DT6_CLIREM,(cAliasDT6)->DT6_LOJREM,cCliExp,cLojExp,cFilOri,cNumSol,aUF,cAmbiente)
						cStringExp	:= grpExpedid((cAliasDT6)->DT6_CLIREM,(cAliasDT6)->DT6_LOJREM,cCliExp,cLojExp,cFilOri,cNumSol,aUF,cAmbiente)
					EndIf
					...
					cQuery += "  MAX(SD1.D1_PICM)  D1_PICM,  " + CRLF
					cQuery += "  SUM(SD1.D1_ICMSRET) D1_ICMSRET  " + CRLF
					...
					cQuery += " AND SFT.FT_CLIEFOR = '" + cClieFor + "'" + CRLF
					...
					While !(cAliasTagJ)->(Eof()) .And. ++nCount <= 1
						...
						ElseIf SubStr((cAliasTagJ)->FT_CLASFIS, 2, 2) $ "10,30,70"
							...
							cString += '</ICMS90>'
						ElseIf SubStr((cAliasTagJ)->FT_CLASFIS, 2, 2) $ '20,90'
							//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
							//³ - 81: ICMS DEVIDOS A OUTRAS UF'S                                ³
							//³ - 90: ICMS Outros                                               ³
							//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
...
Local lDULAtu	:= DUL->( FieldPos("DUL_RECINS") ) > 0 .AND. DUL->( FieldPos("DUL_RECPES") ) > 0 .AND. DUL->( FieldPos("DUL_RECCGC") ) > 0 .AND. DUL->( FieldPos("DUL_IMPREC") )
	...
	If !Empty(cSeekSA1)
		...
		Else
			...
			cCnpjCpf  := IIf( AllTrim(SA1->A1_PESSOA) == "J",NoPontos(SA1->A1_CGC),AllTrim(SA1->A1_CGC))
			lJuridico := (SA1->A1_PESSOA == "J")
			cIE       := IIf(DUL->(Found()) .And. 'ISENT' $ Upper(AllTrim(DUL->DUL_INSCR)),"ISENTO",SA1->A1_INSCR)
		EndIf
		cCONTRIB  := SA1->A1_CONTRIB
		cNomeRec  := If(cAmbiente == '2',SubStr('CT-E EMITIDO EM AMBIENTE DE HOMOLOGACAO - SEM VALOR FISCAL',1,60),AllTrim(SA1->A1_NOME))
		cTelefone := AllTrim(NoAcentoCte(SA1->A1_DDD)) + AllTrim(NoAcentoCte(SA1->A1_TEL))
		cEmail    := AllTrim(SA1->A1_EMAIL)
