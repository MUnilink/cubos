select
	...
	TQS.TQS_KMOR + TQS.TQS_KMR1 + TQS.TQS_KMR2 + TQS.TQS_KMR3 + TQS.TQS_KMR4 + TQS.TQS_KMR5 + TQS.TQS_KMR6 + TQS.TQS_KMR7 /
	isnull
	(	
		(
			(
				select sum(STJ010.TJ_CUSTTER)
				from STJ010 (nolock)
				where
						STJ010.D_E_L_E_T_ = ''
					and STJ010.TJ_CODBEM = STJ.TJ_CODBEM
			) + ST9.T9_VALCPA
		), 1
	) as CPK,
/*
