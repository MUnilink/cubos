select
    ...
    trim(EST.T9_CODBEM) as CM,
    case COM.T9_CATBEM when 3 then null else trim(COM.T9_CODBEM) end as SR,
    case COM.T9_CATBEM when 3 then trim(COM.T9_CODBEM) else null end as PNEU,
...
from STZ010 STZ (nolock)
    inner join ST9010 EST
        ...
        and EST.T9_TEMCONT = 'S'

    left join ST9010 COM
        on COM.D_E_L_E_T_ = ''
