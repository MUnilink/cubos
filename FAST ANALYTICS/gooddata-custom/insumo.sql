select
	...
	trim(isnull(STL.TL_HOINICI, '-')) as TL_HOINICI,
	trim(isnull(STL.TL_HOFIM, '-')) as TL_HOFIM,
	...
	isnull(trim(SA2.A2_NOME), isnull(trim(SB1.B1_DESC), isnull(trim(SH4.H4_DESCRI), isnull(trim(ST0.T0_NOME), isnull(trim(ST1.T1_NOME), '-'))))) as DESC_INSUMO,
