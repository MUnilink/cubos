    select distinct
        ...
        case
            ...
            when cast(ZC2.ZC2_TIPO as int) in (3, 6, 9, 10, 12, 13) then concat(concat(trim(ZC2.ZC2_TIPO), ' ', (select max(trim(ST9010.T9_CODBEM)) from ST9010 (nolock) where ST9010.D_E_L_E_T_ = '' and trim(ST9010.T9_CODBEM) = trim(ZC2.ZC2_COD) and cast(ZC2.ZC2_TIPO as int) in (3, 6, 9, 10, 12, 13)))
