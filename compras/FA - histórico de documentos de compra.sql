SELECT
    ...
    convert(datetime, trim(concat(SCP.CP_DATPRF, ' ', max(SCP.CP_YHORASA) over(partition by SCP.CP_FILIAL, SCP.CP_NUM order by SCP.CP_FILIAL, SCP.CP_NUM))), 113) as DATA_SAPRF,
    convert(datetime, trim(concat(SCP.CP_EMISSAO, ' ', max(SCP.CP_YHORASA) over(partition by SCP.CP_FILIAL, SCP.CP_NUM order by SCP.CP_FILIAL, SCP.CP_NUM))), 113) as DATA_SAEMI,
    convert(datetime, trim(concat(SCP.CP_YDATAPR, ' ', max(SCP.CP_YHORAPR) over(partition by SCP.CP_FILIAL, SCP.CP_NUM order by SCP.CP_FILIAL, SCP.CP_NUM))), 113) as DATA_SAPRERE,
    convert(datetime, trim(concat(SCP.CP_YDATABA, ' ', max(SCP.CP_YHORABA) over(partition by SCP.CP_FILIAL, SCP.CP_NUM order by SCP.CP_FILIAL, SCP.CP_NUM))), 113) as DATA_SABAIXA,
    convert(datetime, trim(concat(SC1.C1_EMISSAO, ' ', max(isnull(nullif(SC1.C1_YHORASC, ''), '00:00:00')) over(partition by SC1.C1_FILIAL, SC1.C1_NUM order by SC1.C1_FILIAL, SC1.C1_NUM))), 113) as DATA_SC,
    convert(datetime, trim(concat(SC7.C7_EMISSAO, ' ', max(isnull(nullif(SC7.C7_YHORAPC, ''), '00:00:00')) over(partition by SC7.C7_FILIAL, SC7.C7_NUM order by SC7.C7_FILIAL, SC7.C7_NUM))), 113) as DATA_EMIPC,
    convert(datetime, trim(concat(SF1.F1_DTDIGIT, ' ', SF1.F1_YHORANF)), 113) as DATA_DIGNF,
    cast(SC7.C7_DATPRF as date) as DATA_PRVPC
